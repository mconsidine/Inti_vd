...
# modif valerie VHD
# ajout fonctions de lecture
# correction bug si gros fichier Height et Width en uint64 
...
class Serfile():
    ...
    def _readExistingHeader(self):
        ...
        try : 
            with open(self._nameOfSerfile, 'rb') as file:
                ...
                header['ImageWidth'] = np.uint64(ImageWidth) # VHD
                offset += 4
                
                file.seek(offset)
                
                ImageHeight = np.fromfile(file, dtype='uint32', count=1)[0]
                header['ImageHeight'] = np.uint64(ImageHeight) # VHD
                offset += 4
                ...
                #######################
                # VHD 
                lengthWithoutTrail = np.uint64(ImageHeight) * ImageWidth * FrameCount * BytesPerPixel + 178
                lengthWithTrail = np.uint64(ImageHeight) * ImageWidth * FrameCount * BytesPerPixel + 178 + 8*FrameCount #SPECIFICATION : trail contain 1 date per frame.
                readOK = True
                trail = False
    ...
    def createFitsHeaderFromFitsFile(self, fitsFile):
    ...
    def addFrames(self, frames):
        """add frames, add one to length. If it is the first frame, set height and width and frame_dimension
        In : numpy array containing the multiple frames
        """
        #try : 
            #self._frameDimension
        #except : 
        ...
        try : 
            if self._header['FrameCount']==0 : #first frame
                self._updateHeader('ImageHeight',frames.shape[1])
                self._updateHeader('ImageWidth',frames.shape[2])
                self._height = self._header.get('ImageHeight')
                self._width = self._header.get('ImageWidth')
                self._frameDimension = self._height * self._width
            ### TODO : vérfiier taille de l'image 
            ...
            self._updateHeader('FrameCount', self._length)
                ...
                raise IndexError("tuple index out of range, may be Width and height are false. Shape :  %s"%(frames.shape))
    ...
    def setFrameCount(self, frame_count):
        self._updateHeader('FrameCount',frame_count)
